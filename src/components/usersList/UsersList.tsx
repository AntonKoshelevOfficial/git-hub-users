...
const UsersList: React.FC<types.TProps> = ({
    handleOnItemClick,
}) => {
    const [ inputValue, setInputValue ] = useState<string>('');
    const [ users, setUsers ] = useState<Array<types.UserItemType>>([
        {
            "login":"fanvsfan",
            "id":30,
    ...
    ]) //TODO mockData for gitHub limit requests
    const handleOnChange = useCallback((event: React.FormEvent<HTMLInputElement>) => {
        setInputValue(event.currentTarget.value)
    }, [])

    useEffect(() => {
        const userListWithReposCount: Array<types.UserItemType> = [];
        // axios.get(`https://api.github.com/users`)
        //     .then(response => {
        //         response.data.forEach((item: types.UserItemType) => {
        //             axios.get(item.repos_url)
        //                 .then(repositories => {
        //                     userListWithReposCount.push({
        //                         ...item,
        //                         repos_count: repositories.data.length,
        //                     })
        //                 })
        //         })
        //     });
        // setUsers(userListWithReposCount)
    }, [])
